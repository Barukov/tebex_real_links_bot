...
def create_checkout_link(username: str, package_id: str) -> str:
    basket_payload = {
        "complete_url": f"https://{TEBEX_STORE_IDENTIFIER}.tebex.io/",
        "cancel_url": f"https://{TEBEX_STORE_IDENTIFIER}.tebex.io/",
        "complete_auto_redirect": False,
        "username": username,
        "custom": {"minecraft_username": username},
    }

    basket = tebex_post(
        f"/accounts/{TEBEX_PUBLIC_TOKEN}/baskets",
        basket_payload,
    ).get("data", {})
    ...
    tebex_post(
        f"/baskets/{basket_ident}/packages",
        {"package_id": str(package_id), "quantity": 1},
    )

    basket_info = tebex_get(
        f"/accounts/{TEBEX_PUBLIC_TOKEN}/baskets/{basket_ident}"
    ).get("data", {})
...
async def cmd_links(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        f"Генерирую {count} ссылок по {price} EUR для ника {username}..."
    )
